...
class AxonUpdaterApp(tk.Tk):
    def __init__(self):
        ...
        # Check root
        if os.geteuid() != 0:
            messagebox.showerror("Permission Denied", "Axon Updater requires administrative privileges. Please run via sudo or pkexec.")
            self.destroy()
            sys.exit(1)
        ...
        self.style.configure("TLabel", background="#2E3440", foreground="#D8DEE9", font=("Inter", 12))
        self.style.configure("Title.TLabel", font=("Inter", 16, "bold"), foreground="#88C0D0")
        self.style.configure("TButton", font=("Inter", 11, "bold"), background="#5E81AC", foreground="white")
        self.style.map("TButton", background=[("active", "#81A1C1")])
        self.style.configure("TProgressbar", thickness=15, troughcolor="#4C566A", background="#A3BE8C")
        ...
        self.lbl_status = ttk.Label(frame, text="Ready to update core OS and sandboxed apps.", wraplength=450, justify=tk.CENTER)
    ...
    def _update_process(self):
        ...
        if not self.run_cmd("timeshift --create --comments 'Axon OS Auto-Update Snapshot'"):
            self.log("Warning: Failed to create snapshot (Timeshift not configured?). Proceeding anyway.", 20)
        else:
        ...
        if not self.run_cmd("DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade -y -q"):
            self.after(0, lambda: messagebox.showerror("Error", "System package update failed. Check terminal logs."))
        ...
        self.after(0, lambda: self.btn_start.config(text="Close", command=self.destroy, state=tk.NORMAL))
